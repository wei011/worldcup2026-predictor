...
**已结束 6 场　·　方向命中 3/6 (50%)　·　平均 Brier score 0.710**（三分类瞎猜基准 0.667，越低越好）

| 日期 | 对阵 | 赛果 | 模型(胜/平/负) | 命中 |
|---|---|---|---|---|
| 06-11 | 墨西哥 vs 南非 | 2:0 | 86% / 10% / 4% | ✅ |
| 06-12 | 韩国 vs 捷克 | 2:1 | 52% / 25% / 22% | ✅ |
| 06-12 | 加拿大 vs 波黑 | 1:1 | 73% / 17% / 10% | ❌ |
| 06-13 | 美国 vs 巴拉圭 | 4:1 | 50% / 26% / 24% | ✅ |
| 06-13 | Qatar vs 瑞士 | 1:1 | 5% / 12% / 83% | ❌ |
| 06-13 | 巴西 vs 摩洛哥 | 1:1 | 46% / 27% / 27% | ❌ |
...
**2. 进球模型**（[src/model.py](src/model.py)）：把 1998 年以来每场比赛拆成 (Elo 差, 实际进球) 样本（约 5.2 万条），分桶 + 加权最小二乘拟合出 λ(d)。比分服从双泊松 → 平局、净胜球、积分规则全部自然涌现，无需单独的"平局模型"。
...
- **换进球模型**：实现 Dixon-Coles 低比分修正，或把身价数据加进 λ
